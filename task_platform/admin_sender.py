...
from .models import Withdraw, ChatVision, Chatinfo, Task, Task_receive
from .tools import chatinfo_num
class Admin_Sender(object):
    ...
    def __state_1(self, message, username, room_id):
        if re.search(self.__funcStr[self.__state][0], message):
            ...
            taskNum=0
            for i in Task.objects.filter(task_state='进行中',publisher=username):
                taskNum+=1
            taskFinish=Task.objects.filter(task_state='完成',publisher=username).count()
            taskAll=Task.objects.filter(task_state='未开始').count()
            noticeNum=chatinfo_num(username)
            self.__send_notice__(
                '''当前正在执行的任务数为:{} \n  
                   已经执行完成的任务数为：{}\n
                   系统当前待执行的任务数为：{}\n
                   当前未查看的信息条数为:{}\n'''.format(taskNum,taskFinish,taskAll,noticeNum),
                username, room_id)
        else:
