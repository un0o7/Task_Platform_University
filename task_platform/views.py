...
from jiebatest import classifier
...
def index(request):
    ...
    # 初始化变量
    tag_list = []
    # 已经登录了才能拿到对应的user-name
    if username:
        user = User.objects.get(name=username)
        student_id = user.stu_id
        phone = user.phone
        dept = user.dept
        notice_room = '/chatroom/{}'.format(get_notice_room_id(username))# 和管理员聊天的窗口
        notice_num = chatinfo_num(username)# 和其他人聊天的窗口数量
        if user.dept == 'None':
            dept = '暂无信息'
        # 更改个人信息
        if request.method == 'POST':
            sl_message = self_settings(request, user)
            # 根据提交的信息更改密码，手机号，学院，这个函数返回的是一个字符串
    # 主页 任务状态图标颜色
    finder = {
        '未开始': '9', '进行中': '2',
        '中止': '3', '撤销': '3', 
...
def search(request):
    ...
    if username:
        ...
        if user.dept == 'None':#dept default='None'
            dept = '暂无信息'
        # 更改个人信息
        if request.method == 'POST':
            sl_message = self_settings(request, user)
    ...
    info = request.GET.get('search').strip()# search的字符串
    # 任务描述中含有该搜索字符串
...
def detail(request, task_id):#点击任务，显示任务的详细信息
    '''
    GET:
    ...
    notice_room = '/chatroom/{}'.format(get_notice_room_id(username))
    notice_num = chatinfo_num(username)# 返回未查看的信息数，并管理员向用户发送已经提现成功的信息
    user = User.objects.get(name=username)
    student_id = user.stu_id
    phone = user.phone
    dept = user.dept
    ...
    # )
    is_publisher = True
    task_receiver=Task_receive.objects.filter(task_id=task.id)
    if username != publisher:
        publisher = '匿名用户：{}'.format(
            random.choice(('金刚鹦鹉', '小牛', '小鹿', '北极熊', '天辉', '夜魇'))
        )
        is_publisher = False
   #之后is_publisher没有再被调用过了
    message = ''
    little_task_list=Task.objects.filter(parent_id=task_id)#这个是子任务
    #对于这个前端需要将这个使用if后进行显示子任务，没有就算了
    user_task_list = User_task.objects.filter(task_id=task_id)# 这个是猎人的报价
    percentage = settings.PERCENTAGE#这个是关于平台收取费用的比例
    # 当前用户是否是接受者的一部分
    is_receiver = (Task_receive.objects.filter(task_id=task.id, username=username).count() != 0)
    #前端应该根据这个receiver来进行区分
    # 判断超时
    ...
    if request.method == "POST":
        if 'settings' in request.POST:
            ...
            if not check_deposit(username, float(settings.DEPOSIT)):
                message = '余额不足，请先充值'
                return render(request, 'task_platform/create-task.html', locals())
                #return redirect('/recharge/') # 余额不足，主要是这样做会失败
            if classifier(task_description):#引入了监督管理机制，对任务描述进行分析
                message='不符合社会主义核心价值观'
                return render(request, 'task_platform/create-task.html', locals())
            ...
            par=request.POST.get("parent_id")#判断是否是在创建子任务
            if par :
                task.parent_id=par
            task.save()
            for tag in tags:#任务标签，输入为含空格的字符串
